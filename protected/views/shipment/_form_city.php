<div class="form">
		...
		<div class="c50l">
			<div class="row">
				...
				<div id="status-radiobutton">
					<?php echo $form->radioButtonList($model, 'type', $model->listtype, array('rel' => CHtml::activeId($model, 'type'))); ?>
					<?php Yii::app()->clientScript->registerScript('autofillberatvolumemetrik', "$('input[rel=" . CHtml::activeId($model, 'type') . "]').change(function(){
					if($('input[rel=" . CHtml::activeId($model, 'type') . "]:checked').val()=='0'){
						$('input#Shipment_package_weight').val('0.5');
						$('div#dimension').hide();
					}
					else {
						$('input#Shipment_package_weight').val('');
						$('div#dimension').show();
					};
					$('input#" . CHtml::activeId($model, 'type') . "').val($(this).val());
					})"); ?>
				</div>
	...
	<div class="row">
		...
		<?php
		echo $form->dropDownList($model_city, 'area_id', CHtml::listData(IntraCityAreas::model()->findAll(), 'id', 'name'), array(
			'prompt' => '--Pilih Rate Area--'
				)
		);
		...
		<?php
		$ajax__get_type = CHtml::ajax(array(
			'url' => array('shipment/intracityService'),
			'type' => 'post',
			'dataType' => 'html',
			'data' => 'js:$("input[name*=ShipmentItem]").serialize()+"&"+$("#ShipmentIntracity_area_id").serialize()',
			'success' => 'js:function(data){
...
$script = <<<EOD
				$("#ShipmentIntracity_area_id").live('change',function(){ $ajax__get_type });
EOD;
$cs->registerScript('_city_area', $script, CClientScript::POS_READY);
